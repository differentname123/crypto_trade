...
def generate_XSR_signals(kline_df, fr_df, bar_minutes=15):
    ...
    高度内聚的回测与信号生成函数：
    1. 策略参数配置统一在开头。
    2. 生成与实盘一致的标准化交易事件流 (OPEN/CLOSE 分离)。
    3. 内部自我包含所有对齐和撮合逻辑，零外部依赖。
    """
    ...
    df['signal_entry'] = rk_ret_M > STRATEGY_PARAMS['ENTRY_RANK_THRESHOLD']

    # 信号 B: 出场
    fr_rank = fr_series.rolling(W, min_periods=mp).rank(pct=True)
    df['signal_exit'] = (fr_rank < STRATEGY_PARAMS['EXIT_FR_RANK_THRESHOLD']) | (fr_series < 0)
    ...
    for i in range(len(df) - 1):
        if not in_pos and df['signal_entry'].iloc[i]:
            ...
            entry_reason = f"SURGE_EXTREME(rank>{STRATEGY_PARAMS['ENTRY_RANK_THRESHOLD']})"
            records.append({
                'time': dt_bj_str,
                'action': 'BUY',
                'coin': coin_name,
            ...
            exit_reason = f"FR_LOW_NEG(rank<{STRATEGY_PARAMS['EXIT_FR_RANK_THRESHOLD']} or fr<0)"
            records.append({
                'time': dt_bj_str,
                'action': 'SELL',
                'coin': coin_name,
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("正在处理数据与回测...")
        trade_records_df = generate_trade_records(kline_df, fr_df, bar_minutes=30)
