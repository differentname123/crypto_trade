...
[数据流转/交互]
  ...
  铁律: 交易所写操作与不可从交易所重建的关键状态变更，必须先写 WAL 落盘；失败即 LedgerError 硬停机。
...
class ExchangeGateway:
    ...
    def load_instrument(self):
        """解析交易规格；拿不到 tick/step 等生命线字段时返回 None，由上层拒绝启动。"""
        try:
            self._throttle()
            return ex_api.fetch_instrument_spec(self.ex, self.symbol)
        except Exception as e:
            logger.error(f"[网关/规格] 获取交易规格失败，无法安全构造价量 | "
                         f"交易对:[{self.symbol}] | 错误:[{e}]")
            return None

    def fetch_last_price(self):
        """返回最新价；同币种进程优先复用短时公共缓存，异常统一降级为 None。"""
        now = time.time()
        if self.shared_prices is not None:
            try:
                cached = self.shared_prices.get(self.symbol) or {}
                cached_ts = float(cached.get("ts") or 0)
                cached_price = float(cached.get("price") or 0)
                if cached_price > 0 and now - cached_ts < SHARED_PRICE_CACHE_SEC:
                    return cached_price
            except Exception:
                pass
        ...
        try:
            self._throttle()
            price = float(ex_api.fetch_last_price(self.ex, self.symbol) or 0)
            if price <= 0:
                return None
            if self.shared_prices is not None:
                self.shared_prices[self.symbol] = {"price": price, "ts": time.time()}
            return price
        except Exception as e:
            logger.error(f"[网关/行情] 拉取最新价失败，本 Tick 不做交易决策 | "
                         f"交易对:[{self.symbol}] | 可能原因:[网络或交易所接口抖动] | 错误:[{e}]")
            return None
    ...
    def fetch_order(self, coid):
        """点查订单：UniOrder=确定快照，ORDER_NOT_FOUND=确定不存在，None=结果未知。"""
        try:
            order, err = ex_api.fetch_order_uni(self.ex, self.symbol, coid, API_THROTTLE_SEC)
        except Exception as e:
            logger.warning(f"[网关/点查] 查询异常，保留原 OID 严禁换号重发 | CID:[{coid}] | "
                           f"可能原因:[网络或交易所接口异常] | 错误:[{e}]")
            return None
        if err is not None:
            logger.info(f"[网关/点查] 结果未知，保留原 OID 下轮继续确认 | CID:[{coid}] | 错误:[{err}]")
            return None
        return order
...
class MartinLedger:
    ...
    def append(self, cycle_id, signal_ts, layer, role, action, coid="",
        ...
        logger.debug(f"[账本/WAL] 事件已落盘 | 动作:[{action}] 周期:[{cycle_id}] 层:[{layer}] "
                     f"角色:[{role}] CID:[{coid}] 价:[{price}] 量:[{qty}] 状态:[{status}] {msg}")
...
class MartinCycle:
    ...
    def _evaluate(self, w):
        ...
        if spec.qty_is_dust(self.book.open_qty):
            ...
            # : 归因仍沿用历史近似口径；若 TP 与 SL 恰好同量成交，可能误归因为外部平仓。
    ...
    def suspend_add(self, why):
        """停止加仓；先写 WAL，再修改内存状态，随后仅维护出场保护。"""
        if self.add_suspended:
            return
        self.wal_cycle(MartinLedger.A_ALERT, MartinLedger.S_SUSPEND_ADD, why)
        self.add_suspended = True
        logger.critical(f"[周期/降级] 已停止加仓，仅维护止盈止损 | 周期:[{self.cycle_id}] | 原因:[{why}]")

    def end_cycle(self, reason, why=""):
        """判定周期终结（幂等）；WAL 成功后才切换到两阶段收尾。"""
        if self.end_reason:
            return
        self.wal_cycle(MartinLedger.A_ALERT, reason.value, why)
        self.end_reason = reason
        self.add_suspended = True
        logger.critical(f"[周期/终结] 已进入收尾：先撤净，再平残余 | 周期:[{self.cycle_id}] "
                        f"原因:[{reason.value}] 持仓:[{self.book.open_qty:.8g}] | 说明:[{why}]")
    ...
    def _abandon(self, slot, why):
        """永久停挂一个槽位；先写 WAL，再修改槽位状态。"""
        if slot.abandoned:
            return
        self.wal_order(MartinLedger.A_ALERT, slot.coid, slot.layer, slot.role.value,
                       status=MartinLedger.S_GIVEUP, msg=why)
        slot.abandoned = True
        logger.critical(f"[订单/放弃] 槽位已永久停挂 | 角色:[{slot.role.value}] 层:[{slot.layer}] "
                        f"尝试:[{slot.attempts}]次 | 原因:[{why}]")
    ...
    def _send_place(self, a):
        """先把确定 OID 的下单意图写入 WAL，再登记内存状态并发出网络请求。"""
    ...
    def _after_place(self, a, t, res):
        ...
        if res.ok:
            ...
            if slot is not None and a.role is not OrderRole.OPEN:
                slot.attempts = 0
            logger.info(f"[订单/挂单] 已受理 | {tag} | 结果:[OK] 交易所单号:[{res.ex_id}]")
            return
        ...
        if res.unknown or res.kind in (ErrKind.UNKNOWN_RESULT, ErrKind.DUPLICATE):
            ...
            logger.critical(f"[订单/挂单] 回执不确定，原地锁定等待点查，严禁换号重发 | "
...
class MartinEngine:
    ...
    def _preview_blueprint(self):
        """按当前价预览多空蓝图，并保留既定的启动确认窗口。"""
        price = self.gw.fetch_last_price()
        if not price:
            logger.warning("[启动/预览] 无法获取最新价，跳过蓝图预览 | 结果:[继续启动]")
            return

        logger.info(f"[启动/预览] 使用当前价生成多空蓝图 | 交易对:[{self.cfg.symbol}] 最新价:[{price}]")
        signal_ts = int(time.time() * 1000)
        for direction in (Direction.LONG, Direction.SHORT):
            BlueprintBuilder.build(self.cfg, self.spec,
                                   Signal(direction, price, signal_ts, "PREVIEW"))

        logger.info(f"[启动/预览] 蓝图已输出，请核对参数；确认窗口:[{STARTUP_PREVIEW_SEC}s] | "
                    "发现参数异常可立即终止进程")
        remaining = STARTUP_PREVIEW_SEC
        while remaining > 0:
            logger.info(f"[启动/预览] 距正式启动:[{remaining}s]")
            sleep_sec = min(10, remaining)
            time.sleep(sleep_sec)
            remaining -= sleep_sec
        logger.info("[启动/预览] 确认窗口结束 | 结果:[继续真实启动流程]")
    ...
    def _idle_tick(self):
        ...
        if not w.algo_ok:
            ...
            return
        if w.orders:
            # : 历史行为会消费当前信号，因此“发现残留挂单”会放弃一次开仓机会。
            # 是否改成“清场确认成功后重试同一信号”属于业务边界，本次不改。
...
def run_single_strategy(cfg, shared_prices=None):
    ...
    try:
        ...
        # : Windows 分支历史上没有 OS 级排他锁，仅创建锁文件；如需 Windows 生产运行，
        # 应先明确跨平台单实例方案。本次不引入新依赖以免改变部署边界。
        ...
        if lock_fd is not None:
            os.close(lock_fd)
        logger.critical(f"[进程/锁] 获取单实例锁失败，拒绝启动以避免账本/OID 冲突 | "
...
def admin_cancel_strategy(exchange, symbol, strategy_id):
    ...
    # : 历史运维工具只遍历普通挂单，无法保证撤掉未触发算法条件单。
    # 为避免改变人工工具边界，本次保留；执行后必须用 admin_inspect 复核。
