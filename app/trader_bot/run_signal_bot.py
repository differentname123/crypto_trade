...
def reconcile_ledger(exchange, ledger, open_order_cache):
    ...
    for symbol in missing_symbols:
        try:
            recent_orders_by_id.update(fetch_recent_orders_map(exchange, symbol, limit=RECENT_ORDER_LIMIT))
        except Exception as exc:
            logger.warning(_log("RECON/BATCH", "近期订单批量查询失败，将由单笔查询兜底", Symbol=symbol, Reason=exc))
            time.sleep(0.5)
    ...
    for _, row in pending.iterrows():
        ...
        if exchange_status == OS_OPEN and is_timeout:
            ...
            try:
                ...
                try:
                    final_view = extract_order_view(fetch_order_by_id(exchange, symbol, exchange_oid))
                    ...
                    logger.warning(_log(
                        "RECON/CANCEL", "撤单已提交，但终态复查失败；按已撤单继续核销",
                        ExchangeOID=exchange_oid, Reason=exc,
                    ))

                # 技术修复：原实现即使复查发现“撤单瞬间已成交”也固定写 CANCELED，现在优先采用复查终态
                ...
                logger.info(_log(
                    ...
                    Event=event, ExchangeOID=exchange_oid,
                    FinalStatus=ledger_status, Filled=filled_amount,
                ))
            except Exception as exc:
                if is_cancel_target_gone(exc):
                    ...
                    timeout_count += 1
                    logger.warning(_log(
                        "RECON/CANCEL", "撤单目标已不存在，按原规则强制核销",
                    ...
                    logger.warning(_log(
                        "RECON/CANCEL", "超时撤单失败，保留 PENDING 等待下轮",
...
def check_position_consistency(ledger, position_cache):
    ...
    for _, row in active_opens.iterrows():
        position_key = make_position_key(str(row["symbol"]).strip(), str(row["direction"]).strip())
        expected_by_position[position_key] = expected_by_position.get(position_key, 0.0) + to_num(row["filled_amount"])
    ...
    for position_key, expected_amount in expected_by_position.items():
        ...
        if actual_amount < expected_amount * (1 - POSITION_DIFF_TOLERANCE):
            logger.warning(_log(
                "RECON/POSITION", "账本理论持仓高于交易所实际持仓，不自动调整",
                Position=position_key, Ledger=f"{expected_amount:.6f}",
                Exchange=f"{actual_amount:.6f}", Tolerance=f"{POSITION_DIFF_TOLERANCE:.2%}",
...
def preload_account_state(exchange, ledger):
    ...
    for attempt in range(1, API_MAX_RETRY + 1):
        try:
            ...
            equity = 0.0
            logger.warning(
                _log("PRELOAD/EQUITY", "权益接口异常，将重试", Attempt=f"{attempt}/{API_MAX_RETRY}", Reason=exc))
    ...
    position_count = "N/A" if position_cache is None else len(position_cache)
    order_count = "N/A" if open_order_cache is None else sum(len(v) for v in open_order_cache.values())

    logger.info(_log(
        "PRELOAD/SUMMARY", "账户快照完成",
        Equity=f"{equity:.2f}", Positions=position_count, OpenOrders=order_count,
        Elapsed=f"{(time.perf_counter() - started_at) * 1000:.0f}ms",
    ))
...
def handle_open(exchange, ledger, ledger_df, sig, total_equity, open_order_cache):
    ...
    if _has_pending_order(open_order_cache, sig):
        logger.warning(_log("OPEN/SKIP", "检测到同信号挂单，阻止重复发单", SSD=ssd_key, Prefix=sig["prefix"]))
        return

    if not _active_opens(ssd_df).empty:
        logger.warning(_log("OPEN/WARN", "本 SSD 仍有未平成交开仓，但按原规则继续执行新信号", SSD=ssd_key,
                            Hint="检查上一对开平信号或人工干预"))
    ...
    if amount <= 0:
        logger.warning(_log("OPEN/SKIP", "精度处理后下单数量为 0", SSD=ssd_key, TargetValue=f"{target_value:.2f}",
                            Price=sig["price"]))
        return

    result = execute_order(
        exchange=exchange, symbol=sig["symbol"], side=sig["action"],
    ...
    log_fn(_log(
        "OPEN/ORDER", "开仓请求已完成",
        SSD=ssd_key, Amount=amount, Value=f"{target_value:.2f}",
        Status=status, ClientOID=sig["client_oid"],
...
def handle_close(exchange, ledger, ledger_df, sig, position_cache, open_order_cache):
    ...
    if open_record is None:
        logger.info(_log("CLOSE/SKIP", "本 SSD 没有可关联的已成交开仓", SSD=ssd_key, Result="不发平仓单"))
        return
    ...
    if actual_position <= 0:
        ...
        logger.warning(_log(
            "CLOSE/RECONCILE", "交易所快照无对应持仓，按原规则逻辑核销",
    ...
    if actual_position < ledger_open_amount * (1 - POSITION_DIFF_TOLERANCE):
        logger.warning(_log(
            "CLOSE/WARN", "实际持仓不足覆盖账本开仓量，将按实际持仓封顶",
            SSD=ssd_key, Ledger=f"{ledger_open_amount:.6f}",
            Exchange=f"{actual_position:.6f}", Hint="检查手动减仓/其它策略/账实差异",
        ))

    amount = amount_to_precision(exchange, sig["symbol"], min(ledger_open_amount, actual_position))

    if amount <= 0:
        logger.warning(_log("CLOSE/SKIP", "精度处理后平仓数量为 0", SSD=ssd_key, LedgerAmount=ledger_open_amount,
    ...
    log_fn(_log(
        "CLOSE/ORDER", "平仓请求已完成",
        SSD=ssd_key, Amount=amount, Status=status,
        OpenRecordID=linked_open_id, ClientOID=sig["client_oid"],
        **({"Reason": result.error_msg} if result.error_msg else {}),
    ))
...
def execute_single_signal(exchange, row, total_equity, position_cache, open_order_cache, ledger):
    ...
    if position_cache is None or open_order_cache is None:
        logger.error(_log("EXEC/SKIP", "账户缓存缺失，拒绝执行当前信号", Reason="持仓或挂单快照不可用"))
        return
    ...
    logger.warning(_log("EXEC/SKIP", "未知事件类型", Event=sig["event"], SSD=sig["ssd_key"]))


def execute_signals(exchange, target_time, total_equity, position_cache, open_order_cache, signal_df, ledger):
    """执行 target_time ± SIGNAL_WINDOW_MIN 内的信号；单信号异常隔离。"""
    started_at = time.perf_counter()
    ...
    for _, row in valid.iterrows():
        try:
            ...
            logger.error(
                _log("EXEC/ERROR", "单信号执行失败，已隔离并继续其它信号", Reason=exc,
                     Hint="检查信号字段/精度/网关返回/账本状态")
                + f"\n{traceback.format_exc()}"
            )

    logger.info(_log(
        "EXEC/SUMMARY", "本轮信号处理完成",
        ValidSignals=f"{len(valid)}笔",
        Elapsed=f"{(time.perf_counter() - started_at) * 1000:.1f}ms",
    ))
...
def print_position_summary(exchange, ledger, open_order_cache=None):
    ...
    if position_cache is None:
        logger.warning(_log("SUMMARY/SKIP", "最新持仓拉取失败，无法输出汇总", Hint="检查账户接口/网络"))
        return

    active_opens = _active_opens(ledger.read())
    if active_opens.empty:
        logger.info(_log("SUMMARY/POSITION", "本轮结束", TheoreticalOpen=0, Result="当前无理论持仓"))
        return
    ...
    logger.info(
        _log("SUMMARY/POSITION", "本轮结束", TheoreticalOpen=f"{len(lines)}笔")
        + " | Details:\n" + "\n".join(lines)
    )
...
def get_top_movers(exchange, top_n=10, mode="top"):
    ...
    changes = pd.Series(fetch_usdt_swap_changes(exchange), dtype="float64").sort_values(ascending=False)
...
def build_monitor_symbols(exchange, position_cache, ledger, top_n, mode):
    ...
    theoretical_symbols = {str(row["symbol"]).strip() for _, row in _active_opens(ledger.read()).iterrows()}
    holding_symbols = list(actual_symbols.intersection(theoretical_symbols))
    ranked_symbols = get_top_movers(exchange, top_n=top_n, mode=mode)

    final_symbols = list(set(ranked_symbols + holding_symbols))

    logger.info(_log(
        "SIGNAL/MONITOR", "本轮监控币种已生成",
        ExchangeHoldings=len(actual_symbols), LedgerHoldings=len(theoretical_symbols),
        StrategyIntersection=len(holding_symbols), FinalCount=len(final_symbols),
        Symbols=final_symbols,
    ))
    return final_symbols


def get_signal_df(exchange, target_time_str, proxy_url, position_cache, ledger):
    """按 CURRENT_SYMBOL 路由信号工作流。"""
    config = STRATEGY_CONFIGS.get(CURRENT_SYMBOL)
    if config is None:
        logger.error(_log("SIGNAL/CONFIG", "未知策略配置，无法生成信号", CURRENT_SYMBOL=CURRENT_SYMBOL))
        return None

    _, _, workflow, rank_mode, top_n = config
    if rank_mode is None:
        return workflow(target_time_str, proxy_url=proxy_url)

    symbols = build_monitor_symbols(exchange, position_cache, ledger, top_n, rank_mode)
    return workflow(target_time_str, symbol_list=symbols, proxy_url=proxy_url)
...
def run_scheduler():
    ...
    logger.info(_log(
        "SCHED/START", "调度系统已启动",
        Strategy=CURRENT_SYMBOL, Account=ACCOUNT_ALIAS, Ledger=LEDGER_FILE,
    ))
        ...
        try:
            ...
            for attempt in range(1, API_MAX_RETRY + 1):
                try:
                    ...
                    logger.error(
                        _log("PRELOAD/ROUND", "账户预加载异常，将重试", Attempt=f"{attempt}/{API_MAX_RETRY}", Reason=exc)
                        + f"\n{traceback.format_exc()}"
                    )
                time.sleep(3)
            ...
            if not preload_ok:
                logger.error(_log(
                    "SCHED/SKIP", "账户快照连续失败，放弃当前轮次",
                    Attempts=API_MAX_RETRY, Next="60 秒后重新进入调度计算",
                ))
                time.sleep(60)
                continue

            signal_df = get_signal_df(exchange, target_time_str, proxy_url, position_cache, ledger)

            if signal_df is not None and not signal_df.empty:
                execute_signals(exchange, next_run, equity, position_cache, open_order_cache, signal_df, ledger)
                logger.info(_log("SCHED/POST", "信号执行阶段结束", Next="盘后对账并刷新持仓汇总"))
            else:
                logger.info(
                    _log("SIGNAL/SUMMARY", "本轮没有可执行信号", Strategy=CURRENT_SYMBOL, Target=target_time_str))

            print_position_summary(exchange, ledger, open_order_cache)

        except Exception as exc:
            logger.error(
                _log("SCHED/RECOVER", "主循环异常，按原高可用策略 30 秒后重新进入循环", Reason=exc,
