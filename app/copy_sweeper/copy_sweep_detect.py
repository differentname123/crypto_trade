...
def _aggregate_trades(trades, time_window_ms=1000):
    ...
    """
    sorted_trades = sorted(trades, key=lambda x: x['orderTime'])
    ...
    for t in sorted_trades:
        ...
        if (t['symbol'] == last['symbol'] and t['side'] == last['side'] and
                t['positionSide'] == last['positionSide'] and (t['orderTime'] - last['orderTime']) <= time_window_ms):
...
def calculate_martingale_rate_simplified(trades, volume_multiplier=1.5):
    ...
    for key, group_trades in groups.items():
        group_trades.sort(key=lambda x: x['orderTime'])
...
def calculate_tail_risk_index(trades):
    ...
    【返回值说明】：
    - summary: 包含单笔最大亏损绝对值、常态单笔盈利中位数、以及最终的尾部风险剥离比。
        ...
        for trade in group_trades:
            if _is_open(trade):
                ...
                if current_opens:
                    ...
                    hold_time_ms = trade['orderTime'] - first_open['orderTime']
...
def calculate_vw_hold_ratio(trades):
    ...
    for trade in clean_trades:
        ...
        if _is_open(trade):
            open_queues[key].append({'qty': trade['executedQty'], 'time': trade['orderTime']})
            ...
            while qty_to_close > 1e-8 and queue:
                ...
                hold_time_ms = trade['orderTime'] - open_order['time']
