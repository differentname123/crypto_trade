...
import { motion, useScroll, AnimatePresence, useMotionValue, useTransform, animate } from 'framer-motion';
import { ChevronsDown, Check, Fingerprint, AlertCircle, ExternalLink, Lock, ShieldAlert, Zap, RefreshCw, Database, Clock, Activity, ArrowLeft, ArrowRight } from 'lucide-react';
...
const FADE = { initial: {opacity:0}, whileInView: {opacity:1}, viewport: {once:true, amount:0.5} };
const DRAW = { initial: {pathLength:0}, whileInView: {pathLength:1}, viewport: {once:true, amount:0.5} };
...
const Reveal=({children,delay=0,y=24,className=''})=>(
  <motion.div className={className} initial={{opacity:0,y}} whileInView={{opacity:1,y:0}} viewport={{once:true,amount:0.35}} transition={{duration:0.8,delay,ease:EASE}}>{children}</motion.div>
);
...
const ChartFrame=({children})=>(
  <Reveal delay={0.12}>
    <figure className="mt-8 overflow-hidden rounded-2xl border p-4" style={{borderColor:HAIR,background:'linear-gradient(180deg,#0F151E,#0B1118)'}}>
      {children}
    </figure>
  </Reveal>
);
...
const GlobalSvgDefs=()=>(
  <svg style={{width:0,height:0,position:'absolute'}} aria-hidden="true">
    <defs>
      <linearGradient id="g-mart" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={GREEN} stopOpacity="0.25"/><stop offset="100%" stopColor={GREEN} stopOpacity="0"/></linearGradient>
      <linearGradient id="g-bear" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={GREEN} stopOpacity="0.25"/><stop offset="100%" stopColor={GREEN} stopOpacity="0"/></linearGradient>
      <linearGradient id="g-bull" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={GREEN} stopOpacity="0.28"/><stop offset="100%" stopColor={GREEN} stopOpacity="0"/></linearGradient>
      <linearGradient id="g-sys" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={GREEN} stopOpacity="0.22"/><stop offset="100%" stopColor={GREEN} stopOpacity="0"/></linearGradient>
      <linearGradient id="g-eq" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={GREEN} stopOpacity="0.3"/><stop offset="100%" stopColor={GREEN} stopOpacity="0"/></linearGradient>
      <linearGradient id="g-fadeRed" x1="0" y1="0" x2="1" y2="0"><stop offset="0%" stopColor={RED} stopOpacity="1"/><stop offset="60%" stopColor={RED} stopOpacity="0.8"/><stop offset="100%" stopColor={INK} stopOpacity="0"/></linearGradient>
      <linearGradient id="g-focusGreen" x1="0" y1="0" x2="1" y2="0"><stop offset="0%" stopColor={GREEN} stopOpacity="0.15"/><stop offset="100%" stopColor={GREEN} stopOpacity="0"/></linearGradient>
      <radialGradient id="g-redCoin" cx="35%" cy="35%" r="65%"><stop offset="0%" stopColor="#ff8a8a"/><stop offset="100%" stopColor={RED}/></radialGradient>
      <radialGradient id="g-greenCoin" cx="35%" cy="35%" r="65%"><stop offset="0%" stopColor="#7affca"/><stop offset="100%" stopColor={GREEN}/></radialGradient>
    </defs>
  </svg>
);
...
const MartingaleChart=()=>{
  ...
  return(
    <svg viewBox="0 0 320 200" className="w-full h-56">
      ...
      <motion.path d={poly(r)+' L 245 184 L 280 184'} fill="none" stroke={RED} strokeWidth="2.2" strokeLinecap="round" {...DRAW} transition={{duration:1.8,ease:'easeInOut'}}/>
      ...
      <motion.circle cx="263" cy="184" r="4" fill="none" stroke={RED} strokeWidth="1.5" initial={{opacity:0}} whileInView={{opacity:[0.8,0],scale:[1,3]}} viewport={{once:true,amount:0.5}} transition={{delay:1.9,duration:1.4,repeat:Infinity}}/>
      <motion.text x="280" y="171" textAnchor="end" style={{fontFamily:MONO}} fontSize="11" fill={RED} {...FADE} transition={{delay:2}}>−100% 归零</motion.text>
      <motion.text x="58" y="136" style={{fontFamily:MONO}} fontSize="11" fill={RED} {...FADE} transition={{delay:0.8}}>马丁策略</motion.text>
      <motion.path d={area(g,175)} fill="url(#g-mart)" {...FADE} transition={{duration:1.8,delay:0.4}}/>
      <motion.path d={poly(g)} fill="none" stroke={GREEN} strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" {...DRAW} transition={{duration:1.8,ease:'easeInOut',delay:0.4}}/>
      <motion.text x="300" y="38" textAnchor="end" style={{fontFamily:MONO}} fontSize="11" fill={GREEN} {...FADE} transition={{delay:2.2}}>本策略 · 阶梯向上</motion.text>
    </svg>
  );
};
...
const RecoveryChart = () => {
  ...
  return (
    <svg viewBox="0 0 320 230" className="w-full h-64">
      ...
      <motion.line x1={cx} y1="35" x2={cx} y2="225" stroke={HAIR} strokeDasharray="2 3" initial={{pathLength:0}} whileInView={{pathLength:1}} transition={{duration:1}} />
      {rows.map((r, i) => {
        const y = 55 + i * 48;
        return (
          <g key={i}>
            {r.focus && <motion.rect x="0" y={y-22} width="320" height="42" fill="url(#g-focusGreen)" initial={{opacity:0}} whileInView={{opacity:1}} transition={{delay:0.5}} />}
            <text x={cx + 10} y={y - 12} style={{fontFamily:SANS}} fontSize="12" fill={r.focus ? GREEN : TXT} opacity={r.focus ? 1 : 0.6}>{r.label}</text>
            <motion.rect x={cx - r.downW} y={y - 5} width={r.downW} height="10" rx="3" fill={r.focus ? 'rgba(52,224,161,0.4)' : 'rgba(239,91,91,0.4)'} style={{ transformOrigin: 'right' }} initial={{ scaleX: 0 }} whileInView={{ scaleX: 1 }} viewport={{ once: true, amount: 0.5 }} transition={{ duration: 0.8, delay: i * 0.15 + 0.2, ease: 'easeOut' }}/>
            <motion.text x={cx - r.downW - 8} y={y + 4} style={{fontFamily:MONO}} fontSize="11" fill={r.focus ? GREEN : RED} textAnchor="end" initial={{ opacity: 0, x: 5 }} whileInView={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.15 + 0.6 }}>−{r.down}%</motion.text>
            <motion.rect x={cx} y={y - 5} width={r.upW} height="10" rx="3" fill={r.focus ? GREEN : (r.isMax ? 'url(#g-fadeRed)' : TXT)} style={{ transformOrigin: 'left' }} initial={{ scaleX: 0 }} whileInView={{ scaleX: 1 }} viewport={{ once: true, amount: 0.5 }} transition={{ duration: 1.2, delay: i * 0.15 + 0.3, type: 'spring', bounce: 0.25 }}/>
            <motion.text x={cx + r.upW + (r.isMax ? 0 : 8)} y={y + 4} style={{fontFamily:MONO}} fontSize="11" fill={r.focus ? GREEN : (r.isMax ? RED : TXT)} textAnchor="start" initial={{ opacity: 0, x: -5 }} whileInView={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.15 + 0.8 }}>+{r.up}% {r.isMax && <tspan fill={RED} fontSize="14" dy="2">∞</tspan>}</motion.text>
          </g>
        )
      })}
    </svg>
...
const BalanceScale=()=>{
  const loss=[[50,108],[62,108],[74,108],[55,98],[67,98],[61,89],[44,116],[80,116]], gain=[[236,104],[262,104],[249,82]];
  return(
    <svg viewBox="0 0 320 200" className="w-full h-56">
      ...
      <motion.g style={{transformBox:'view-box',transformOrigin:'160px 118px'}} initial={{rotate:0}} whileInView={{rotate:7}} viewport={{once:true,amount:0.5}} transition={{delay:1,duration:1.2,ease:EASE}}>
        <rect x="40" y="116" width="240" height="5" rx="2.5" fill={TXT} opacity="0.85"/>
        <circle cx="160" cy="118" r="6" fill={INK} stroke={TXT}/>
        {loss.map((p,i)=>(<motion.circle key={'l'+i} cx={p[0]} cy={p[1]} r="6" fill="url(#g-redCoin)" initial={{opacity:0,cy:p[1]-30}} whileInView={{opacity:0.9,cy:p[1]}} viewport={{once:true,amount:0.5}} transition={{delay:0.1+i*0.06,type:'spring',stiffness:200,damping:14}}/>))}
        {gain.map((p,i)=>(<motion.circle key={'g'+i} cx={p[0]} cy={p[1]} r="13" fill="url(#g-greenCoin)" initial={{opacity:0,cy:p[1]-30}} whileInView={{opacity:0.95,cy:p[1]}} viewport={{once:true,amount:0.5}} transition={{delay:0.3+i*0.12,type:'spring',stiffness:200,damping:14}}/>))}
      </motion.g>
...
const AreaChart = ({ type }) => {
  ...
  const bench = isBear ? [[10,45],[55,55],[105,72],[155,95],[205,114],[255,126],[290,131]] : [[10,128],[55,118],[105,108],[155,98],[205,90],[255,84],[290,80]];
  const strat = isBear ? [[10,45],[55,42],[105,46],[155,40],[205,44],[255,37],[290,34]] : [[10,128],[50,118],[90,104],[130,86],[170,64],[210,44],[250,28],[290,16]];
  return (
    <svg viewBox="0 0 300 150" className="w-full h-40">
      ...
      <motion.path d={area(strat, 140)} fill={`url(#g-${type})`} {...FADE} transition={{duration:1, delay: isBear ? 0.3 : 0.6}}/>
      <motion.path d={smooth(bench)} fill="none" stroke={isBear ? RED : DIM} strokeWidth={isBear ? 2 : 1.6} strokeDasharray={isBear ? "none" : "4 4"} {...DRAW} transition={{duration:1.4, ease: isBear ? 'easeInOut' : undefined}}/>
      <motion.path d={smooth(strat)} fill="none" stroke={GREEN} strokeWidth={isBear ? 2.2 : 2.4} strokeLinecap={isBear ? undefined : 'round'} {...DRAW} transition={{duration: isBear ? 1.4 : 1.6, ease:'easeInOut', delay:0.3}}/>
    </svg>
  );
};
...
const BullBearTabs = () => {
  ...
  useEffect(() => {
    const timer = setInterval(() => setTab(prev => prev === 'bear' ? 'bull' : 'bear'), 3000);
    return () => clearInterval(timer);
  }, [tab]);
  ...
  return (
    <div className="flex flex-col">
      <div className="mb-6 flex w-full rounded-lg bg-[#0F151E] p-1 border border-white/5">
        {['bear', 'bull'].map(t => (
           <button key={t} onClick={()=>setTab(t)} className={`flex-1 rounded-md py-2 text-xs font-bold tracking-widest transition-colors ${tab===t?'bg-[#34E0A1]/15 text-[#34E0A1]':'text-[#8A93A3]'}`}>{t === 'bear' ? '熊市不亏' : '牛市起飞'}</button>
        ))}
      </div>
        ...
        <AnimatePresence mode="wait">
          <motion.div key={tab} initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-10}} transition={{duration:0.3}}>
            <div className="mb-2 flex items-center justify-end">
              <span style={{fontFamily:MONO,color:DIM}} className="text-xs">
                基准 <span style={{color: tab==='bear'?RED:GREEN}}>{tab==='bear'?'−38.2%':'+120%'}</span> · 策略 <span style={{color:GREEN}}>{tab==='bear'?'+6.5%':'+312%'}</span>
              </span>
            </div>
            <AreaChart type={tab} />
            {tab === 'bear' && <p style={{fontFamily:MONO,color:GREEN}} className="mt-2 text-xs">↑ 超额收益 +44.7%</p>}
          </motion.div>
        </AnimatePresence>
      </div>
    </div>
...
const SystematicChart=()=>{
  ...
  return(
    <svg viewBox="0 0 300 150" className="w-full h-44">
      ...
      <motion.path d={area(line,135)} fill="url(#g-sys)" {...FADE} transition={{duration:1,delay:0.8}}/>
      <motion.path d={smooth(line)} fill="none" stroke={GREEN} strokeWidth="2.4" strokeLinecap="round" {...DRAW} transition={{duration:1.8,ease:'easeInOut'}}/>
      {line.map((p,i)=>(<motion.circle key={i} cx={p[0]} cy={p[1]} r="2.6" fill={INK} stroke={GREEN} strokeWidth="1.5" initial={{opacity:0,scale:0}} whileInView={{opacity:1,scale:1}} viewport={{once:true,amount:0.5}} transition={{delay:0.5+i*0.13}}/>))}
    </svg>
  );
};
...
const Hero=({ hasVisited, onSkip })=>(
  <section className="relative flex min-h-screen flex-col justify-center py-24 mx-auto w-full max-w-md px-6" style={{ scrollSnapAlign: 'center', scrollSnapStop: 'always' }}>
    ...
    <motion.h2 initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{duration:0.8,ease:EASE}} style={{fontFamily:SERIF,color:TXT}} className="mb-8 text-3xl font-medium tracking-wide">怎样才能<span style={{color:GOLD, textShadow:'0 0 20px rgba(231,200,132,0.4)'}}>赚钱</span>？</motion.h2>
...
const Principle=({idx,zh,maxim,takeaway,children})=>(
  <section className="flex min-h-screen flex-col justify-center py-24 mx-auto w-full max-w-md px-6" style={{ scrollSnapAlign: 'center', scrollSnapStop: 'always' }}>
    ...
    <Reveal delay={0.05}><h2 style={{fontFamily:SERIF,color:TXT}} className="mt-7 text-3xl font-semibold leading-snug tracking-wide">{maxim}</h2></Reveal>
    <ChartFrame>{children}</ChartFrame>
    <Reveal delay={0.1}>
      <div className="mt-7 inline-flex items-center gap-3 border-l-[3px] py-2.5 pl-4 pr-6" style={{borderColor: GREEN, background: 'linear-gradient(90deg, rgba(52,224,161,0.1) 0%, transparent 100%)'}}>
...
const Finale=({ onViewRadar })=>{
  ...
  const valRounded = useTransform(valMotion, v => v.toFixed(1));
  ...
  return(
    <section className="relative flex min-h-screen flex-col justify-center py-24 mx-auto w-full max-w-md px-6" style={{ scrollSnapAlign: 'center', scrollSnapStop: 'always' }}>
      ...
      <AnimatePresence mode="wait">
      {!done?(
          <motion.div key="pre" exit={{opacity: 0, scale: 0.96}} transition={{duration: 0.5}} className="flex flex-col items-center text-center w-full">
              <Reveal><p style={{fontFamily: SERIF, color: TXT}} className="text-4xl font-semibold leading-snug">五条法则，<br/>一条曲线。</p></Reveal>
              <Reveal delay={0.1}><p style={{color: DIM}} className="mt-5 text-base leading-relaxed">按住下方，<br/>见证它穿越牛熊。</p></Reveal>
              <div className="mt-8 mb-8 flex w-full flex-col gap-2.5">
                  {feats.map((f, i) => (
                      <motion.div key={i} animate={{opacity: i < revealed ? 1 : 0.18}} transition={{duration: 0.4}} className="flex items-center gap-3 rounded-xl border px-4 py-2.5" style={{ borderColor: i < revealed ? 'rgba(52,224,161,0.3)' : HAIR, background: i < revealed ? 'rgba(52,224,161,0.06)' : 'transparent' }}>
                          <motion.span animate={{scale: i < revealed ? 1 : 0.6}} className="flex h-5 w-5 items-center justify-center rounded-full" style={{ background: i < revealed ? GREEN : 'transparent', border: i < revealed ? 'none' : `1px solid ${HAIR}` }}>
              ...
              <button onPointerDown={start} onClick={start} onContextMenu={e => e.preventDefault()} onTouchStart={e => { }} ref={buttonRef} style={{ touchAction: 'none', userSelect: 'none', WebkitUserSelect: 'none', WebkitTouchCallout: 'none', WebkitTapHighlightColor: 'transparent', borderColor: GREEN, transform: 'translateZ(0)', boxShadow: `0 0 ${15 + pRef.current * 20}px rgba(52,224,161,${0.15 + pRef.current * 0.2})` }} className="relative w-full overflow-hidden rounded-full border-2 px-6 py-4 cursor-pointer">
                  <span ref={progressSpanRef} className="absolute inset-y-0 left-0" style={{ width: `${pRef.current * 100}%`, background: 'rgba(52,224,161,0.22)' }}/>
                  <span className="relative flex items-center justify-center gap-2" style={{color: GREEN, transform: 'translateZ(0)'}}>
                    <Fingerprint size={18}/>
                    <span className="text-base font-semibold tracking-wide">{holding ? '正在揭晓…' : '按住不放 · 见证穿越牛熊的威力'}</span>
                  </span>
              </button>
              <motion.p animate={{opacity: holding ? 0.4 : 0.7}} style={{fontFamily: MONO, color: DIM}} className="mt-4 text-xs tracking-widest">{holding ? 'LOADING ···' : 'PRESS & HOLD · 长按解锁'}</motion.p>
          </motion.div>
      ) : (
          <motion.div key="post" initial={{opacity: 0, scale: 0.96}} animate={{opacity: 1, scale: 1}} transition={{duration: 0.6, ease: EASE}} className="w-full">
              ...
              <div className="mt-2 flex items-end gap-1">
                  ...
                  <motion.span style={{fontFamily: MONO, color: GOLD, textShadow: '0 0 40px rgba(231,200,132,0.4)'}} className="text-6xl font-bold tracking-tight tabular-nums">{valRounded}</motion.span>
...
const fetchSignals = async () => {
  ...
  const displayHistory = showHistory ? signalData.historyPositions : signalData.historyPositions.slice(0, 3);
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }} className="min-h-screen py-10 px-4 mx-auto w-full max-w-md">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2 text-[#8A93A3] text-xs">
          ...
          <button onClick={fetchSignals} disabled={isLoading} className={`p-1.5 rounded bg-[#3C82F6]/20 text-[#3C82F6] hover:bg-[#3C82F6]/30 transition-colors ${isLoading ? 'opacity-50 cursor-not-allowed' : ''}`}>
      ...
      <div className="grid grid-cols-2 gap-3 mb-8">
        <div className="bg-[#0F151E] border border-white/5 rounded-xl p-3 shadow-sm">
          <div className="flex items-center justify-between mb-2"><div className="flex items-center gap-1 text-[#E7C884] text-xs"><Database size={12}/>累计收益</div></div>
        ...
        <div className="bg-[#0F151E] border border-white/5 rounded-xl p-3 shadow-sm">
          ...
          <div className="text-[#BCC2CE] text-[10px] leading-tight whitespace-pre-line" style={{fontFamily: MONO}}>{signalData.stats.timeRange}</div>
        </div>
      </div>
            ...
            return (
              <div key={idx} className="bg-[#0F151E] border border-y-white/5 border-r-white/5 rounded-lg p-4 shadow-sm" style={{ borderLeft: `3px solid ${themeColor}` }}>
                ...
                <div className="flex bg-[#161C24] rounded-lg p-3">
                  <div className="flex-1"><div className="text-[#8A93A3] text-[10px] mb-1">开仓价格 (USDT)</div><div className="text-white font-bold" style={{fontFamily: MONO}}>{pos.price}</div></div>
                  <div className="flex-1"><div className="text-[#8A93A3] text-[10px] mb-1">仓位占比</div><div className="text-[#3C82F6] font-bold" style={{fontFamily: MONO}}>{pos.size}</div></div>
                </div>
              </div>
            )
          })
      ...
      <div className="mb-10 rounded-xl border p-5 shadow-lg" style={{borderColor: 'rgba(52,224,161,0.25)', background: 'linear-gradient(135deg, rgba(52,224,161,0.06) 0%, transparent 100%)'}}>
        <div className="flex items-center gap-2 mb-3">
          <Zap size={16} color={GREEN}/>
          <span style={{fontFamily: MONO, color: GREEN}} className="text-sm font-bold tracking-widest">SYSTEM UPGRADE | 自动化实盘接入</span>
        </div>
        <p className="text-xs text-[#BCC2CE] mb-4 leading-relaxed">当前页面信号免费公开，但人工盯盘必定产生时间滑点。为实现毫秒级系统执行，现免费开放 Alpha 节点 API 自动化交易权限申请。</p>
        <button onClick={() => {setShowWarningModal(true); setModalStep(1); setAgreed(false);}} style={{background: GREEN, color: INK}} className="flex w-full items-center justify-center gap-2 rounded-lg py-3 text-[14px] font-bold transition-transform active:scale-[0.98]">
          申请 API 自动化对接权限 <ArrowRight size={16} />
        </button>
      </div>
      ...
      <div className="flex flex-col gap-3 mb-12">
            ...
            return (
              <div key={idx} className="bg-[#0F151E] border border-white/5 rounded-lg p-4 shadow-sm">
                ...
                <div className="flex justify-between items-end">
                  <div><div className="text-[#8A93A3] text-[10px] mb-1">开仓 · {pos.openTime}</div><div className="text-[#BCC2CE] text-xs" style={{fontFamily: MONO}}>{pos.openPrice}</div></div>
                  <div className="w-px h-6 bg-white/5 mx-2" />
                  <div className="text-right"><div className="text-[#8A93A3] text-[10px] mb-1">平仓 · {pos.closeTime}</div><div className="text-[#BCC2CE] text-xs" style={{fontFamily: MONO}}>{pos.closePrice}</div></div>
                </div>
              </div>
            )
          })
        ...
        {!showHistory && signalData.historyPositions.length > 3 && (
          <button onClick={() => setShowHistory(true)} className="w-full rounded-lg border border-white/5 bg-white/[0.02] py-3 text-xs text-[#8A93A3] hover:text-white transition-colors mt-2">
            展开全部历史交易记录
          </button>
        )}
      </div>
      ...
      <AnimatePresence>
        {showWarningModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-6 backdrop-blur-md" style={{ background: 'rgba(10, 14, 20, 0.85)' }}>
            <motion.div initial={{ scale: 0.95, y: 10 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 10 }} className="relative w-full max-w-sm overflow-hidden rounded-2xl border border-white/10 bg-[#0A0E14] shadow-2xl">

              {modalStep === 1 ? (
                <>
                  <div className="absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-transparent via-[#EF5B5B] to-transparent opacity-50"></div>
                  <div className="flex flex-col p-6 text-left">
                    <div className="mb-4 flex items-center gap-2"><ShieldAlert size={20} color={RED}/><span style={{fontFamily:MONO, color:RED}} className="text-base font-bold tracking-widest">SYSTEM WARNING<br/>接入须知</span></div>

                    <ul className="flex flex-col gap-4 mb-6">
                      {[
                        {t:'低频交易', d:'大量时间空仓，拒绝高频无效操作，不适合追求高频刺激的用户。'},
                        {t:'接受止损', d:'用连续的小额亏损，换取单次大赚。靠高盈亏比与大趋势获利。'},
                        {t:'机器纪律', d:'放弃短期暴富幻想，绝对服从系统。做时间的朋友。'},
                        {t:'顺势而为', d:'不抄底不逃顶，只吃确定性的波段，专注长线复利。'}
                      ].map((item, i) => (
                        <li key={i} className="flex items-start gap-2.5">
                          <div className="mt-1.5 flex h-1.5 w-1.5 shrink-0 rounded-full bg-[#EF5B5B]/80"></div>
                          <div className="flex flex-col">
                            <span style={{color:TXT}} className="text-[14px] font-bold tracking-wide">{item.t}</span>
                            <span style={{color:DIM}} className="text-[12px] mt-0.5 leading-relaxed">{item.d}</span>
                          </div>
                        </li>
                      ))}
                    </ul>

                    <div className="mb-6 flex cursor-pointer items-center gap-3 rounded-lg p-3 transition-colors bg-white/[0.03] border border-white/5" onClick={()=>setAgreed(!agreed)}>
                      <div className={`flex h-4 w-4 shrink-0 items-center justify-center rounded border transition-all ${agreed ? 'border-[#34E0A1] bg-[#34E0A1]/20' : 'border-[#8A93A3]/50'}`}>
                        {agreed && <Check size={12} color={GREEN} strokeWidth={4}/>}
                      </div>
                      <p style={{color: DIM}} className="text-[12px]">我已理解：交易是一场反人性的修行，感受时间复利魅力。</p>
                    </div>
                    ...
                    <button onClick={agreed ? handleApply : undefined} disabled={!agreed} style={{ background: agreed ? GREEN : 'rgba(255,255,255,0.05)', color: agreed ? INK : DIM, opacity: agreed ? 1 : 0.5}} className={`w-full flex items-center justify-center gap-2 rounded-xl py-3.5 text-[15px] font-bold transition-all ${agreed ? 'active:scale-[0.98] shadow-[0_0_15px_rgba(52,224,161,0.2)]' : 'cursor-not-allowed'}`}>
                       {agreed ? <Zap size={18} /> : <Lock size={18} />} 获取自动化接入
                    </button>
                    <button onClick={() => setShowWarningModal(false)} className="mt-4 text-xs text-center w-full text-[#8A93A3] hover:text-white transition-colors">暂不接入，关闭窗口</button>
                  </div>
                </>
              ) : (
                <>
...
export default function App(){
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      setHasVisited(localStorage.getItem('hasVisitedRadar') === 'true');
    }
  }, []);

  const goToRadar = () => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('hasVisitedRadar', 'true');
    }
    setHasVisited(true);
    setView('radar');
  };
    ...
    <div style={{background:INK,color:TXT,fontFamily:SANS}} className="relative h-screen w-full overflow-hidden">
      ...
      {view === 'landing' ? (
        <>
          ...
          <div ref={scrollRef} className="relative z-10 h-full w-full overflow-y-auto overflow-x-hidden" style={{ scrollSnapType: 'y mandatory' }}>
            <Hero hasVisited={hasVisited} onSkip={goToRadar}/>
            <Principle idx="01" zh="捕捉趋势" maxim="市场不可预测，规则方可长青。" takeaway="不预测，只跟随趋势"><MartingaleChart/></Principle>
            <Principle idx="02" zh="控制回撤" maxim="先求不死，再求大胜。" takeaway="回撤可控，绝不致命"><RecoveryChart/></Principle>
            <Principle idx="03" zh="及时止损" maxim="截断亏损，让利润奔跑。" takeaway="亏有底线，赢无上限"><BalanceScale/></Principle>
            <Principle idx="04" zh="穿越牛熊" maxim="牛市决定收益，熊市决定复利。" takeaway="熊市不亏，牛市起飞"><BullBearTabs/></Principle>
            <Principle idx="05" zh="系统驱动" maxim="把交易交给系统，把情绪踢出局。" takeaway="规则驱动，透明可验"><SystematicChart/><RuleStatus/></Principle>
            <Finale onViewRadar={goToRadar}/>
          </div>
        </>
      ) : (
        <div className="relative z-10 h-full w-full overflow-y-auto overflow-x-hidden">
          <AnimatePresence mode="wait"><SignalRadar onBack={() => setView('landing')} /></AnimatePresence>
        </div>
      )}
    </div>
  );
