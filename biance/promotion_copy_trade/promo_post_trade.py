...
#             -> 严格校验双视角JSON -> 回写 promo_post 字段。
#   3. [发布] DB 拉带评论帖 -> 组装带引流链接的评论 -> comment_on_binance_post 发帖 -> 回写 promo_post_info。
# [输出数据]: 副作用为主 —— 向 MongoDB 帖子文档追加 promo_post(生成结果) 与 promo_post_info(发布状态)。
# ==========================================
import datetime
import re
import time
...
PROMPT_FILE_PATH = r'W:\project\python_project\crypto_trade\prompt\biance 分析帖子推广评论生成.txt'
USER_DATA_DIR = r"W:\temp\biance_jie"
LEAD_DETAIL_URL = "https://www.binance.com/zh-CN/square/post/352727546639281?_ul=aHR0cHM6Ly9hcHAuYmluYW5jZS5jb20vdW5pLXFyL2Nwb3MvMzUyNzI3NTQ2NjM5MjgxP3VjPWFwcF9zcXVhcmVfc2hhcmVfbGluayZ1cz1jb3B5bGluayZyPVROOTBPT1Q5Jmw9emgtQ04mdWNvPU9IdTFSLXRkR212QTVfY19LazhoNXc&ref=TN90OOT9&utm_campaign=app_square_share_link&utm_content=OHu1R-tdGmvA5_c_Kk8h5w&utm_source=copylink&sqb=1"
...
def check_comment_info(data):
    ...
    # 1. 校验顶层 post_analysis 字段
    if "post_analysis" not in data:
        return False, "缺失顶层字段: post_analysis"

    post_analysis = data["post_analysis"]
    if not isinstance(post_analysis, dict):
        return False, "post_analysis 必须是字典结构"

    for pa_field in ["post_type", "is_desert", "quoted_details"]:
        if pa_field not in post_analysis:
            return False, f"post_analysis 缺失必要字段: {pa_field}"

    if not isinstance(post_analysis.get("quoted_details"), list):
        return False, "post_analysis.quoted_details 必须是列表类型"

    # 2. 校验两个核心评论生成角色
    required_fields = [
        "menu_choice", "structure_used",
        "comment_text", "link_text", "combined_preview",
        "score", "score_tier", "score_reason"
    ]
    # 绝对禁止的终止性标点
    forbidden_endings = ("。", "！", "？", ".", "!", "?")
    valid_score_tiers = ("HIGH", "MID", "LOW")

    for perspective in ("empathetic_user", "rational_user"):
        ...
        score_tier = view_data["score_tier"]
        ...
        # 业务潜规则 1：评论文案长度 20-60 字
        if not (20 <= len(comment_text_stripped) <= 60):
            return False, f"{perspective}.comment_text 违规：长度必须在 20-60 字之间，当前为 {len(comment_text_stripped)} 字"

        # 业务潜规则 2：评论文案不得以终止性标点收尾
        if comment_text_stripped.endswith(forbidden_endings):
            return False, f"{perspective}.comment_text 违规：绝对禁止以句号、感叹号、问号结尾"

        # --- link_text 校验 ---
        if not isinstance(link_text, str):
            return False, f"{perspective}.link_text 类型非字符串"
        # 业务潜规则 3：引流文案长度严格 2-6 个字
        if not (2 <= len(link_text.strip()) <= 10):
        ...
        if score_tier not in valid_score_tiers:
            return False, f"{perspective}.score_tier 违规：档位必须是 HIGH/MID/LOW 之一，当前为 {score_tier}"
...
def clear_all_promo_posts_batch():
    ...
    posts_to_update = []
    for post in existing_posts:
        if "promo_post" in post:
            post["promo_post"] = None
...
def send_single_promo_post(post):
    ...
    # 卫语句：无评论 / 已处理过 直接跳过
    comment_info = post.get("promo_post")
    if not comment_info or "promo_post_info" in post:
        return None
